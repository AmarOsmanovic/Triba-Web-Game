const canvas = document.getElementById("canvasPiramida");
...
for (let i = 0; i < 13; i++) {
    for (let j = i; j < 13 - i; j++) {
        const x = canvas.offsetWidth / 25 + j * canvas.offsetWidth / 13;
        const y = canvas.offsetHeight / 25 + i * canvas.offsetHeight / 13;
        const r = canvas.offsetWidth / 100;
        const start = 0;
        const end = 2 * Math.PI;
        ctx.beginPath();
...
canvas.addEventListener("click", function (event) {
    ...
    for (let i = 0; i < 13; i++) {
        for (let j = i; j < 13 - i; j++) {
            const kruzicX = canvas.offsetWidth / 25 + j * canvas.offsetWidth / 13;
            const kruzicY = canvas.offsetHeight / 25 + i * canvas.offsetHeight / 13;
            const r = canvas.offsetWidth / 100;
