const canvas = document.getElementById("canvas18x15");
...
for (let i = 0; i < 18; i++) {
    for (let j = 0; j < 15; j++) {
        const x = canvas.offsetWidth / 30 + i * canvas.offsetWidth / 15;
        const y = canvas.offsetHeight / 30 + j * canvas.offsetHeight / 15;
        const r = canvas.offsetWidth / 120;
        const start = 0;
        const end = 2 * Math.PI;
        ctx.beginPath();
...
canvas.addEventListener("click", function (event) {
    ...
    for (let i = 0; i < 6; i++) {
        for (let j = 0; j < 5; j++) {
            const kruzicX = canvas.offsetWidth / 30 + i * canvas.offsetWidth / 15;
            const kruzicY = canvas.offsetHeight / 30 + j * canvas.offsetHeight / 15;
            const r = canvas.offsetWidth / 120;
