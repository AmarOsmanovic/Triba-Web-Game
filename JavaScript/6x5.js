const canvas = document.getElementById("canvas6x5");
...
for (let i = 0; i < 6; i++) {
    for (let j = 0; j < 5; j++) {
        const x = canvas.offsetWidth / 10 + i * canvas.offsetWidth / 5;
        const y = canvas.offsetHeight / 10 + j * canvas.offsetHeight / 5;
        const r = canvas.offsetWidth / 40;
        const start = 0;
        const end = 2 * Math.PI;
        ctx.beginPath();
...
canvas.addEventListener("click", function (event) {
    ...
    for (let i = 0; i < 6; i++) {
        for (let j = 0; j < 5; j++) {
            const kruzicX = canvas.offsetWidth / 10 + i * canvas.offsetWidth / 5;
            const kruzicY = canvas.offsetHeight / 10 + j * canvas.offsetHeight / 5;
            const r = canvas.offsetWidth / 40;
